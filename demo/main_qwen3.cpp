...
int main(int argc, char* argv[]) {
  if (argc < 3 || argc > 5) {
    LOG(INFO) << "Usage: ./demo checkpoint path tokenizer path [cuda|cpu] [total steps]";
    return -1;
  }
  const char* checkpoint_path = argv[1];  // e.g. out/model.bin
  const char* tokenizer_path = argv[2];
  const std::string device_name = argc >= 4 ? argv[3] : "cuda";
  const int total_steps = argc >= 5 ? std::stoi(argv[4]) : 2560;

  base::DeviceType device_type = base::DeviceType::kDeviceCUDA;
  if (device_name == "cpu") {
    device_type = base::DeviceType::kDeviceCPU;
  } else if (device_name != "cuda") {
    LOG(FATAL) << "Unsupported device: " << device_name << ", expected cuda or cpu.";
  }
